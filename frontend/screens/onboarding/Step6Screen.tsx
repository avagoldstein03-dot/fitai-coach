...
export default function OnboardingStep6() {
  ...
  const { mutate: submit, isPending } = useMutation({
    mutationFn: async () => {
      await axios.post(`${API_URL}/api/onboarding/step6`, {
        foodAllergies: allergies ? allergies.split(",").map((a) => a.trim()) : [],
      });
    },
    onSuccess: () => navigation.navigate("Step7"),
  });
  ...
  return (
    <ScrollView style={s.screen} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <View style={s.container}>
        <Text style={s.heading}>{t("onboarding.step6.title")}</Text>
        <Text style={s.subtitle}>{t("onboarding.step6.subtitle")}</Text>
        <TextInput
