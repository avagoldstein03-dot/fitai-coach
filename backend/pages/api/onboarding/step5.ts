...
import type { OnboardingStep5Request } from "@/types/index";
import { z } from "zod";
...
export default async function handler(
  ...
  try {
    ...
    const validationResult = step5Schema.safeParse(req.body);
    if (!validationResult.success) {
      return sendError(res, "validation_error", "Invalid request body", 400);
    }

    const data = validationResult.data as OnboardingStep5Request;
